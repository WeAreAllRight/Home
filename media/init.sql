...
create database hms charset=utf8;
...
(
   ...
   m_user               VARCHAR(50),
   m_concent            VARCHAR(256),
   m_notice             VARCHAR(256),
   m_advertisement      VARCHAR(256),
   m_msg                VARCHAR(256),
   m_illegal_user       VARCHAR(20),
   m_illegal_content    VARCHAR(256),
   primary key (m_id)
);
...
alter table t_message_feedback add constraint FK_Reference_46 foreign key (m_id)
